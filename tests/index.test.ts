...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['NEPTUNE_API_V2_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: NeptuneAPIV2) => {
    ...
    test('debug logs when log level is debug', async () => {
      ...
      const client = new NeptuneAPIV2({ logger: logger, logLevel: 'debug' });
    ...
    test('debug logs are skipped when log level is info', async () => {
      ...
      const client = new NeptuneAPIV2({ logger: logger, logLevel: 'info' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['NEPTUNE_API_V2_LOG'] = 'debug';
      const client = new NeptuneAPIV2({ logger: logger });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['NEPTUNE_API_V2_LOG'] = 'not a log level';
      const client = new NeptuneAPIV2({ logger: logger });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'NEPTUNE_API_V2_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
    ...
    test('client log level overrides env var', async () => {
      ...
      process.env['NEPTUNE_API_V2_LOG'] = 'debug';
      const client = new NeptuneAPIV2({ logger: logger, logLevel: 'off' });
  ...
  test('explicit global fetch', async () => {
    ...
    const client = new NeptuneAPIV2({ baseURL: 'http://localhost:5000/', fetch: defaultFetch });
  });
  ...
  test('normalized method', async () => {
    ...
    const client = new NeptuneAPIV2({ baseURL: 'http://localhost:5000/', fetch: testFetch });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new NeptuneAPIV2({ baseURL: 'http://localhost:5000/custom/path/' });
    ...
    afterEach(() => {
      process.env['NEPTUNE_API_V2_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new NeptuneAPIV2({ baseURL: 'https://example.com' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['NEPTUNE_API_V2_BASE_URL'] = 'https://example.com/from_env';
      const client = new NeptuneAPIV2({});
    ...
    test('in request options overridden by client options', () => {
      const client = new NeptuneAPIV2({ baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
    ...
    test('in request options overridden by env variable', () => {
      process.env['NEPTUNE_API_V2_BASE_URL'] = 'http://localhost:5000/env';
      const client = new NeptuneAPIV2({});
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new NeptuneAPIV2({ maxRetries: 4 });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new NeptuneAPIV2({});
    expect(client2.maxRetries).toEqual(2);
  });

  describe('withOptions', () => {
    test('creates a new client with overridden options', async () => {
      const client = new NeptuneAPIV2({ baseURL: 'http://localhost:5000/', maxRetries: 3 });
    ...
    test('respects runtime property changes when creating new client', () => {
      const client = new NeptuneAPIV2({ baseURL: 'http://localhost:5000/', timeout: 1000 });
...
describe('retries', () => {
  test('retry on timeout', async () => {
    ...
    const client = new NeptuneAPIV2({ timeout: 10, fetch: testFetch });
  ...
  test('retry count header', async () => {
    ...
    const client = new NeptuneAPIV2({ fetch: testFetch, maxRetries: 4 });
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new NeptuneAPIV2({ fetch: testFetch });
