...
describe('resource debts', () => {
  ...
  test.skip('list', async () => {
    const responsePromise = client.markets.borrow.debts.list();
  ...
  test.skip('getByAsset: only required params', async () => {
    const responsePromise = client.markets.borrow.debts.getByAsset({ asset_id: 'token;-K-//-//3-' });
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
  ...
  test.skip('getByAsset: required and optional params', async () => {
    const response = await client.markets.borrow.debts.getByAsset({
      asset_id: 'token;-K-//-//3-',
      with_text: true,
      with_value: true,
    });
