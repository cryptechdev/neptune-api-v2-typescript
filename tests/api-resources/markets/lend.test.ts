...
describe('resource lend', () => {
  ...
  test.skip('list', async () => {
    const responsePromise = client.markets.lend.list();
  ...
  test.skip('list: request options and params are passed correctly', async () => {
    ...
    await expect(
      client.markets.lend.list({ with_text: true, with_value: true }, { path: '/_stainless_unknown_path' }),
    ).rejects.toThrow(NeptuneAPIV2.NotFoundError);
  });

  // Mock server tests are disabled
  test.skip('getByAsset: only required params', async () => {
    const responsePromise = client.markets.lend.getByAsset({ asset_id: 'token;-K-//-//3-' });
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
  ...
  test.skip('getByAsset: required and optional params', async () => {
    const response = await client.markets.lend.getByAsset({
      asset_id: 'token;-K-//-//3-',
      with_text: true,
      with_value: true,
    });
  ...
  test.skip('getRateHistory: only required params', async () => {
    const responsePromise = client.markets.lend.getRateHistory({
      end: 0,
      period: 'h',
      start: 0,
    });
  ...
  test.skip('getRateHistory: required and optional params', async () => {
    const response = await client.markets.lend.getRateHistory({
      end: 0,
      period: 'h',
      start: 0,
