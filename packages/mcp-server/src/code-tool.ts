...
import { ClientOptions } from '@neptunefinance/api-v2';

const prompt = `Runs JavaScript code to interact with the Neptune API V2 API.
  ...
  const res = await fetch(codeModeEndpoint, {
    ...
    headers: {
      ...
      'x-stainless-mcp-client-envs': JSON.stringify({
        NEPTUNE_API_V2_BASE_URL: readEnv('NEPTUNE_API_V2_BASE_URL') ?? client.baseURL ?? undefined,
      }),
    },
    body: JSON.stringify({
      project_name: 'neptune-api-v2',
      code,
      intent,
      client_opts: {},
...
const localDenoHandler = async ({
  ...
  try {
    const sdkPkgName = '@neptunefinance/api-v2';
