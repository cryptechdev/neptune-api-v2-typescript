...
async function fetchLatestInstructions(stainlessApiKey: string | undefined): Promise<string> {
  ...
  const response = await fetch(
    readEnv('CODE_MODE_INSTRUCTIONS_URL') ?? 'https://api.stainless.com/api/ai/instructions/neptune-api-v2',
    {
      method: 'GET',
  ...
  if (!response.ok) {
    ...
    instructions = `
      This is the neptune-api-v2 MCP server. You will use Code Mode to help the user perform
